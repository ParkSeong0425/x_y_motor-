...
#define R_DI3     0x0306  // DI3 기능 설정 (H03_06, 0: 기능 없음, 배선 없음)
#define R_DI3L    0x0307  // DI3 로직 설정 (H03_07, 0: 정논리)
#define R_DI5     0x030A  // DI5 기능 설정 (H03_10, 34: 비상정지)
#define R_DI5L    0x030B  // DI5 로직 설정 (H03_11, 0: 평소, 1: PF7 눌림 -> 비상정지)
...
#define R_HMODE   0x051E  // H05_30, 6: 현재 위치를 원점(0)으로 설정
...
int motor_init(Motor *m, int dir) {
	...
	if (!write16(m, R_DI3, 0))
		return 0;      // DI3 기능 없음 (배선 없음)
	if (!write16(m, R_DI3L, 0))
		return 0;
	if (!write16(m, R_DI5, 34))
		return 0;      // DI5 = 비상정지 (PF7 버튼 배선)
	if (!write16(m, R_DI5L, 0))
		return 0;      // 평소값 0, PF7 누르면 button.c가 1로 바꿈
...
// ===== 원점 잡기: 지정 방향으로 100rpm 이동, PF9(정지) 누른 자리가 원점(0) =====
int motor_home(Motor *m, int target) {
	pause = 0;
	motor_move(m, 100, target);

	uint32_t t0 = HAL_GetTick();
	while (HAL_GetTick() - t0 < 120000) {   // 최대 2분
		if (estop) {
			motor_stop(m);
			return 0;
		}
		...
		if (HAL_UART_Receive(&huart3, &c, 1, 100) == HAL_OK)
			pause = 1;                      // 아무 글자나 입력하면 정지
		if (pause) {                        // 정지한 자리 = 원점
			motor_stop(m);
			HAL_Delay(100);
			print("Home set\r\n");
			return write16(m, R_HMODE, 6);  // 현재 위치를 0으로
		}
	}
	motor_stop(m);
	print("Timeout\r\n");
...
int motor_wait(Motor *mx, int xtarget, Motor *my, int ytarget, int timeout) {
	...
	while (HAL_GetTick() - t0 < (uint32_t) timeout) {
		if (pause || estop) {            // 버튼 정지 요청: 두 모터 즉시 정지
			motor_stop(mx);
			motor_stop(my);
			print("Stop\r\n");
			return 0;
		...
		uint8_t c;
		if (HAL_UART_Receive(&huart3, &c, 1, 200) == HAL_OK)
			pause = 1;                      // 아무 글자나 입력하면 정지
	}
	print("Timeout\r\n");
	return 0;
